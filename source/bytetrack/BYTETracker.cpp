...
vector<STrack> BYTETracker::update(const vector<TrackObject>& objects)
{
	...
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); i++)
		{
			...
			strack.detbox[0] = objects[i].rect.x;
			strack.detbox[1] = objects[i].rect.y;
			strack.detbox[2] = objects[i].rect.width;
			strack.detbox[3] = objects[i].rect.height;
	...
	for (int i = 0; i < this->tracked_stracks.size(); i++)
	{
		if (this->tracked_stracks[i].is_activated)
		{
			this->tracked_stracks[i].bLost= false;
...
std::vector<STrack> BYTETracker::getLostSTrack()
{
	std::vector<STrack> out_lost_stracks;
	for (auto& strack : this->lost_stracks)
	{
		if ((strack.is_activated == true) && (strack.state == TrackState::Lost))
		{
			out_lost_stracks.push_back(strack);
		}
	}
	return out_lost_stracks;
}
std::vector<STrack> BYTETracker::getSTrack()
{
