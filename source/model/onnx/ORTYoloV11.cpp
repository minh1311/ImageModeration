...
ORTYoloV11::ORTYoloV11(nlohmann::json& jModelConfig)
    : ORTModel(jModelConfig, JN_MODEL_YOLOV11)
{
    m_iWidthModel = umpInputTensorsShape[m_strInputName][3];
    m_iHeightModel = umpInputTensorsShape[m_strInputName][2];
    
    m_iOutputHeight = 4 + m_iNumClasses;
...
ORTYoloV11::~ORTYoloV11()
{
    
}

void ORTYoloV11::run(cv::Mat& mImage, std::vector<stObject_t>& stObjects)
{
    preprocess(mImage);
...
void ORTYoloV11::preprocess(cv::Mat& mImage)
{
...
void ORTYoloV11::postprocess(std::vector<stObject_t>& stObjects)
{
    stObjects.clear();
